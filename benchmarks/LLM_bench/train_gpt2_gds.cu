...
void gds_close_file(CUfileHandle_t* cf_handle, int fd)
{
	cuFileHandleDeregister(*cf_handle);
	close(fd);
}
int gds_open_file(const char* filename)
{
    int fd = -1;
...
{
    int ret = -1;
    ret = cuFileWrite(*cf_handle, dev_buffer, io_size, file_offset, devBuf_offset);
    if(ret < 0)
    {
...
int gds_file_to_device(CUfileHandle_t* cf_handle, void* dev_buffer, size_t io_size, off_t file_offset, off_t devBuf_offset)
{
    int ret = -1;
    ret = cuFileRead(*cf_handle, dev_buffer, io_size, file_offset, devBuf_offset);
    if(ret < 0)
    {
...
// END GDS
void gpt2_cuFileWrite_to_checkpoint(GPT2 *model, const char* checkpoint_path) {
    ...
    gds_device_to_file(cf_handle, model->params_memory, model->num_parameters_bytes, 0, 0);    
    gds_close_file(cf_handle, fd);
...
void gpt2_forward(GPT2 *model, const int* inputs, size_t B, size_t T, int step) {
    ...
    // float total_time_forward = 0.0;
    // float total_offload_time = 0.0;   

    // cudaEventCreate(&start_forward);
    // cudaEventCreate(&stop_forward);
    // cudaEventRecord(start_forward);
    ...
    // cudaEventRecord(stop_forward);
    // cudaEventSynchronize(stop_forward);
    // cudaEventElapsedTime(&total_time_forward, start_forward, stop_forward);
    // printf0("Total forward time: %.3f s,  offload activations time: %.3f s, compute time: %.3f s\n", total_time_forward / 1000,  total_offload_time / 1000, (total_time_forward-total_offload_time) / 1000);
}
...
void gpt2_backward_and_reduce(GPT2 *model, int* inputs, const int* targets, int grad_accum_steps, int micro_step, int step) {
    ...
    // cudaEventCreate(&stop_backward);
    // cudaEventRecord(start_backward);
    ...
    for (int l = L-1; l >= 0; l--) {
        ...
        if (Enable_offload)
        {
            ...
            if(step > 0)
                total_load_time += time_load / 1000.0f;
    ...
    // cudaEventRecord(stop_backward);
    // cudaEventSynchronize(stop_backward);
    // cudaEventElapsedTime(&total_backward_time, start_backward, stop_backward);
    // printf0("Total forward time: %.3f s,  load activations time: %.3f s, compute time: %.3f s\n", total_backward_time / 1000,  total_load_time / 1000, (total_backward_time-total_load_time) / 1000);
}
...
void load_state(int* step, GPT2* model, DataLoader* loader, const char* filename) {
    ...
    *step = state_header[10]; // step of the optimization
    ...
    gds_file_to_device(cf_handle, model->m_memory, shard_num_parameters * sizeof(float), 0, 0);
    gds_file_to_device(cf_handle, model->v_memory, shard_num_parameters * sizeof(float), 0, 0);
    
    if(model->use_master_weights) {
        assert(model->master_weights != nullptr);
        // file_to_device(model->master_weights, state_file, shard_num_parameters * sizeof(float), IO_BUF_SIZE, main_stream);
        
        gds_file_to_device(cf_handle, model->master_weights, shard_num_parameters * sizeof(float), 0, 0);
...
void write_checkpoint(const char* output_log_dir, int step, GPT2* model, DataLoader* train_loader, MultiGpuConfig* multi_gpu_config) {
    ...
    if (rank == 0) {
        ...
        gpt2_cuFileWrite_to_checkpoint(model, filename_buffer);
    }
...
int main(int argc, char *argv[]) {
    ...
    int process_rank = 1;  // this should be set by the slurm environment
    ...
    if (resuming == 1) {
        ...
        // read checkpoint using gds
        cudaEvent_t load_model_param_start, load_model_param_stop;
        float load_model_time = 0.0;
        cudaEventCreate(&load_model_param_start);
        cudaEventCreate(&load_model_param_stop);
        cudaEventRecord(load_model_param_start);

        gpt2_build_from_checkpoint(&model, load_filename);
        
        cudaEventRecord(load_model_param_stop);
        cudaEventSynchronize(load_model_param_stop);
        cudaEventElapsedTime(&load_model_time, load_model_param_start, load_model_param_stop);
        printf0("Load file: %s, build model from checkpoint time: %.3f ms\n",  load_filename, load_model_time);
    ...
    for (; step <= train_num_batches; step++) {
        ...
        if ((checkpoint_every > 0 && output_log_dir != NULL && resuming == 0) &&
            ...
            // GDS Write
            cudaEvent_t checkpoint_start, checkpoint_stop;
            float checkpoint_time = 0.0;
            cudaEventCreate(&checkpoint_start);
            cudaEventCreate(&checkpoint_stop);
            cudaEventRecord(checkpoint_start);

            write_checkpoint(output_log_dir, step, &model, &train_loader, &multi_gpu_config);
            
            cudaEventRecord(checkpoint_stop);
            cudaEventSynchronize(checkpoint_stop);
  ...
  // size_t off = 0;
            // gds_device_to_file(acts_handle, acts.encoded, sizeof(floatX*), off, 0);
            // off += sizeof(floatX*);
            // gds_device_to_file(acts_handle, acts.ln1, sizeof(floatX*), off, 0);
            // off += sizeof(floatX*);
            // gds_device_to_file(acts_handle, acts.ln1_mean, sizeof(float*), off, 0);
            // off += sizeof(float*);
            // gds_device_to_file(acts_handle, acts.ln1_rstd, sizeof(float*), off, 0);
            // off += sizeof(float*);
            // gds_device_to_file(acts_handle, acts.atty, sizeof(floatX*), off, 0);
            // off += sizeof(floatX*);
            // #if ENABLE_CUDNN
            //     gds_device_to_file(acts_handle, acts.att, sizeof(float*), off, 0); 
            //     off += sizeof(float*);
            // #else
            //     gds_device_to_file(acts_handle, acts.att, sizeof(floatX*), off, 0);
            //     off += sizeof(floatX*);
            
            // gds_device_to_file(acts_handle, acts.residual2, sizeof(floatX*), off, 0);
            // off += sizeof(floatX*);
            // gds_device_to_file(acts_handle, acts.ln2, sizeof(floatX*), off, 0);
            // off += sizeof(floatX*);
            // gds_device_to_file(acts_handle, acts.ln2_mean, sizeof(float*), off, 0);
            // off += sizeof(float*);
            // gds_device_to_file(acts_handle, acts.ln2_rstd, sizeof(float*), off, 0);
            // off += sizeof(float*);
            // gds_device_to_file(acts_handle, acts.fch, sizeof(floatX*), off, 0);
            // off += sizeof(floatX*);
            // gds_device_to_file(acts_handle, acts.fch_gelu, sizeof(floatX*), off, 0);
            // off += sizeof(floatX*);
            // gds_device_to_file(acts_handle, acts.residual3, sizeof(floatX*), off, 0);
            // off += sizeof(floatX*);
            // gds_device_to_file(acts_handle, acts.lnf, sizeof(floatX*), off, 0);
            // off += sizeof(floatX*);
            // gds_device_to_file(acts_handle, acts.lnf_mean, sizeof(float*), off, 0);
            // off += sizeof(float*);
            // gds_device_to_file(acts_handle, acts.lnf_rstd, sizeof(float*), off, 0);
            // off += sizeof(float*);
            // gds_device_to_file(acts_handle, acts.losses, sizeof(float*), off, 0);
            // off += sizeof(float*);
            // gds_device_to_file(acts_handle, acts.qkvr, sizeof(floatX*), off, 0);
            // off += sizeof(floatX*);
            // gds_device_to_file(acts_handle, acts.output, sizeof(floatX*), off, 0);
            // off += sizeof(floatX*);
            // gds_device_to_file(acts_handle, acts.scratch_bt4c, sizeof(floatX*), off, 0);
            // off += sizeof(floatX*);
            // gds_device_to_file(acts_handle, acts.scratch_btc, sizeof(floatX*), off, 0);
            // off += sizeof(floatX*);

            // gds_device_to_file(param_handle, params.wte, sizeof(floatX*), 0, 0);
            // gds_device_to_file(param_handle, params.wpe, sizeof(floatX*), 1 * sizeof(floatX*), 0);
            // gds_device_to_file(param_handle, params.ln1w, sizeof(floatX*), 2 * sizeof(floatX*), 0);
            // gds_device_to_file(param_handle, params.ln1b, sizeof(floatX*), 3 * sizeof(floatX*), 0);
            // gds_device_to_file(param_handle, params.qkvw, sizeof(floatX*), 4 * sizeof(floatX*), 0);
            // gds_device_to_file(param_handle, params.qkvb, sizeof(floatX*), 5 * sizeof(floatX*), 0);
            // gds_device_to_file(param_handle, params.attprojw, sizeof(floatX*), 6 * sizeof(floatX*), 0);
            // gds_device_to_file(param_handle, params.attprojb, sizeof(floatX*), 7 * sizeof(floatX*), 0);
            // gds_device_to_file(param_handle, params.ln2w, sizeof(floatX*), 8 * sizeof(floatX*), 0);
            // gds_device_to_file(param_handle, params.ln2b, sizeof(floatX*), 9 * sizeof(floatX*), 0);
            // gds_device_to_file(param_handle, params.fcw, sizeof(floatX*), 10 * sizeof(floatX*), 0);
            // gds_device_to_file(param_handle, params.fcb, sizeof(floatX*), 11 * sizeof(floatX*), 0);
            // gds_device_to_file(param_handle, params.fcprojw, sizeof(floatX*), 12 * sizeof(floatX*), 0);
            // gds_device_to_file(param_handle, params.fcprojb, sizeof(floatX*), 13 * sizeof(floatX*), 0);
            // gds_device_to_file(param_handle, params.lnfw, sizeof(floatX*), 14 * sizeof(floatX*), 0);
            // gds_device_to_file(param_handle, params.lnfb, sizeof(floatX*), 15 * sizeof(floatX*), 0);
...
// read
  // file_off = 0;
            // for (int i = 0; i < NUM_PARAMETER_TENSORS; i++)
            // {
            //     gds_file_to_device(acts_handle, *(model->params[i].ptr), file_off, 0);
            //     file_off += model->acts_specs[i].size;
            // }
            
            // size_t off = 0;
            // // TODO: model->acts_memory算下偏移量,好像也不需要
            // gds_file_to_device(acts_handle, acts.encoded, sizeof(floatX*), off, 0);
            // off += sizeof(floatX*);
            // gds_file_to_device(acts_handle, acts.ln1, sizeof(floatX*), off, 0);
            // off += sizeof(floatX*);
            // gds_file_to_device(acts_handle, acts.ln1_mean, sizeof(float*), off, 0);
            // off += sizeof(float*);
            // gds_file_to_device(acts_handle, acts.ln1_rstd, sizeof(float*), off, 0);
            // off += sizeof(float*);
            // gds_file_to_device(acts_handle, acts.atty, sizeof(floatX*), off, 0);
            // off += sizeof(floatX*);
            // if(Enable_CUDNN){
            //     gds_file_to_device(acts_handle, acts.att, sizeof(float*), off, 0); 
            //     off += sizeof(float*);
            // }
            // else{
            //     gds_file_to_device(acts_handle, acts.att, sizeof(floatX*), off, 0);
            //     off += sizeof(floatX*);
            // }
                
            // gds_file_to_device(acts_handle, acts.residual2, sizeof(floatX*), off, 0);
            // off += sizeof(floatX*);
            // gds_file_to_device(acts_handle, acts.ln2, sizeof(floatX*), off, 0);
            // off += sizeof(floatX*);
            // gds_file_to_device(acts_handle, acts.ln2_mean, sizeof(float*), off, 0);
            // off += sizeof(float*);
            // gds_file_to_device(acts_handle, acts.ln2_rstd, sizeof(float*), off, 0);
            // off += sizeof(float*);
            // gds_file_to_device(acts_handle, acts.fch, sizeof(floatX*), off, 0);
            // off += sizeof(floatX*);
            // gds_file_to_device(acts_handle, acts.fch_gelu, sizeof(floatX*), off, 0);
            // off += sizeof(floatX*);
            // gds_file_to_device(acts_handle, acts.residual3, sizeof(floatX*), off, 0);
            // off += sizeof(floatX*);
            // gds_file_to_device(acts_handle, acts.lnf, sizeof(floatX*), off, 0);
            // off += sizeof(floatX*);
            // gds_file_to_device(acts_handle, acts.lnf_mean, sizeof(float*), off, 0);
            // off += sizeof(float*);
            // gds_file_to_device(acts_handle, acts.lnf_rstd, sizeof(float*), off, 0);
            // off += sizeof(float*);
            // gds_file_to_device(acts_handle, acts.losses, sizeof(float*), off, 0);
            // off += sizeof(float*);
            // gds_file_to_device(acts_handle, acts.qkvr, sizeof(floatX*), off, 0);
            // off += sizeof(floatX*);
            // gds_file_to_device(acts_handle, acts.output, sizeof(floatX*), off, 0);
            // off += sizeof(floatX*);
            // gds_file_to_device(acts_handle, acts.scratch_bt4c, sizeof(floatX*), off, 0);
            // off += sizeof(floatX*);
            // gds_file_to_device(acts_handle, acts.scratch_btc, sizeof(floatX*), off, 0);
            // off += sizeof(floatX*);

            // gds_file_to_device(param_handle, params.wte, sizeof(floatX*), 0, 0);
            // gds_file_to_device(param_handle, params.wpe, sizeof(floatX*), 1 * sizeof(floatX*), 0);
            // gds_file_to_device(param_handle, params.ln1w, sizeof(floatX*), 2 * sizeof(floatX*), 0);
            // gds_file_to_device(param_handle, params.ln1b, sizeof(floatX*), 3 * sizeof(floatX*), 0);
            // gds_file_to_device(param_handle, params.qkvw, sizeof(floatX*), 4 * sizeof(floatX*), 0);
            // gds_file_to_device(param_handle, params.qkvb, sizeof(floatX*), 5 * sizeof(floatX*), 0);
            // gds_file_to_device(param_handle, params.attprojw, sizeof(floatX*), 6 * sizeof(floatX*), 0);
            // gds_file_to_device(param_handle, params.attprojb, sizeof(floatX*), 7 * sizeof(floatX*), 0);
            // gds_file_to_device(param_handle, params.ln2w, sizeof(floatX*), 8 * sizeof(floatX*), 0);
            // gds_file_to_device(param_handle, params.ln2b, sizeof(floatX*), 9 * sizeof(floatX*), 0);
            // gds_file_to_device(param_handle, params.fcw, sizeof(floatX*), 10 * sizeof(floatX*), 0);
            // gds_file_to_device(param_handle, params.fcb, sizeof(floatX*), 11 * sizeof(floatX*), 0);
            // gds_file_to_device(param_handle, params.fcprojw, sizeof(floatX*), 12 * sizeof(floatX*), 0);
            // gds_file_to_device(param_handle, params.fcprojb, sizeof(floatX*), 13 * sizeof(floatX*), 0);
            // gds_file_to_device(param_handle, params.lnfw, sizeof(floatX*), 14 * sizeof(floatX*), 0);
            // gds_file_to_device(param_handle, params.lnfb, sizeof(floatX*), 15 * sizeof(floatX*), 0);