...
void test_device_write_latency(host_fd_t host_fd, int iterations, int nblocks, int nthreads, size_t log_dev_page_size)
{
    dev_page_size = (1ull << log_dev_page_size);
    uint64_t chunk_size = dev_page_size;
    // preprare data to write 
    srand(time(0));
    int rand_start = rand();
    uint64_t *host_buf = (uint64_t *)malloc(chunk_size);
    for (size_t i = 0; i < chunk_size / sizeof(uint64_t); i++)
        host_buf[i] = rand_start + i;
    
    uint64_t *dev_buf;
    CUDA_SAFE_CALL(cudaMalloc(&dev_buf, chunk_size));
    ...
    {
        // nblocks*nthreads write chunk_size each
        for (vaddr_t offset = 0; offset < virtual_space_size; offset+=chunk_size)
        {
            device_xfer_geminifs_file<<<nblocks, nthreads>>>(dev_fd, offset, dev_buf, chunk_size, 0);
        }
...
int main(int argc, char** argv)
{
    ...
    // device write data to file
    test_device_write_latency(host_fd, iterations, nblocks, nthreads, log_dev_page_size);
